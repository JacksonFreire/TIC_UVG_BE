...
public class ReportService {
	...
	public ResponseEntity<byte[]> generateCourseParticipantReport(Long courseId) throws IOException {
		...
		headersResp.set(HttpHeaders.CONTENT_DISPOSITION,
				"attachment; filename=participantes_curso_" + courseId + ".xlsx");
		return ResponseEntity.ok().headers(headersResp).contentType(MediaType.APPLICATION_OCTET_STREAM)
				.body(out.toByteArray());
	}

	public ResponseEntity<byte[]> generateEventParticipantReport(Long eventId) throws IOException {
		// Obtener todas las inscripciones del evento, incluyendo todos los estados
		List<Enrollment> enrollments = enrollmentRepository.findByEventId(eventId);

		// Verificar que existan inscripciones y que el evento esté definido
		if (enrollments.isEmpty() || enrollments.get(0).getEvent() == null) {
			throw new IllegalArgumentException("No se encontraron inscripciones o el evento no existe.");
		}

		String eventName = enrollments.get(0).getEvent().getName();

		// Crear el libro de Excel y una hoja para los participantes
		Workbook workbook = new XSSFWorkbook();
		Sheet sheet = workbook.createSheet("Participantes Evento");

		// Encabezado con el nombre de la empresa
		String titleText = "UNIVERITAS GROUP - Reporte de Participantes en Evento";
		Row titleRow = sheet.createRow(0);
		Cell titleCell = titleRow.createCell(0);
		titleCell.setCellValue(titleText);
		...
		eventFont.setFontHeightInPoints((short) 12);
		eventFont.setColor(IndexedColors.DARK_BLUE.getIndex());
		...
		HttpHeaders headersResp = new HttpHeaders();
		headersResp.set(HttpHeaders.CONTENT_DISPOSITION,
				"attachment; filename=participantes_evento_" + eventId + ".xlsx");
		return ResponseEntity.ok().headers(headersResp).contentType(MediaType.APPLICATION_OCTET_STREAM)
				.body(out.toByteArray());
	}
}
